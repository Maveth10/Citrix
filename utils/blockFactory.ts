export const createBlock = (type: string, variant: string, label: string) => {
  ...
  // FIX V18.40: Markowa Typografia
  if (type === 'h1') { 
    newBlock.text = 'Wielki Nagłówek H1'; newBlock.styles.fontSize = '48px'; newBlock.styles.fontWeight = '900'; newBlock.styles.margin = '0 0 15px 0';
    if(variant === 'brand'){ newBlock.text = 'MARKOWY TYTUŁ'; newBlock.styles.color='#3b82f6'; newBlock.styles.textTransform='uppercase'; newBlock.styles.letterSpacing = '-0.02em'; } 
    ...
    if(variant === 'outline'){ newBlock.text = 'Pusty w Środku'; newBlock.styles.color = 'transparent'; newBlock.styles.WebkitTextStroke = '2px #0f172a'; }
    if(variant === 'highlight'){ newBlock.text = 'Kluczowy <span style="background: linear-gradient(120deg, rgba(253, 224, 71, 0.8) 0%, rgba(253, 224, 71, 0.8) 100%) no-repeat; background-size: 100% 35%; background-position: 0 90%;">Wyróżnik</span>'; }
  }
  
  if (type === 'h2') { 
    newBlock.text = 'Podtytuł H2'; newBlock.styles.fontSize = '32px'; newBlock.styles.fontWeight = '700'; newBlock.styles.margin = '0 0 15px 0';
  ...
  if (type === 'p') { 
    newBlock.text = 'Zwykły, standardowy blok tekstu. Idealny do dłuższych opisów na stronie.'; newBlock.styles.fontSize = '16px'; newBlock.styles.margin = '0 0 15px 0'; newBlock.styles.lineHeight = '1.6';
    if(variant === 'eyebrow'){ newBlock.text = 'ZACZNIJ TUTAJ'; newBlock.styles.color = '#3b82f6'; newBlock.styles.fontWeight = '800'; newBlock.styles.fontSize = '12px'; newBlock.styles.letterSpacing = '0.15em'; newBlock.styles.textTransform = 'uppercase'; }
    if(variant === 'lead'){ newBlock.text = 'To jest elegancki, markowy akapit tekstu (Lead). Zbudowany większym fontem, w delikatniejszym kolorze. Idealny pod główny nagłówek.'; newBlock.styles.fontSize = '20px'; newBlock.styles.color = '#64748b'; newBlock.styles.lineHeight = '1.7'; }
    if(variant === 'quote'){ newBlock.text = '"Wybitny design to taki, z którego nie można już nic zabrać."'; newBlock.styles.fontStyle = 'italic'; newBlock.styles.borderLeft = '4px solid #3b82f6'; newBlock.styles.paddingLeft = '20px'; newBlock.styles.color = '#475569'; newBlock.styles.fontSize = '18px'; }
  }
