export const createBlock = (type: string, variant: string, label: string) => {
  ...
  // ==========================================
  // 1. TYPOGRAFIA (NAGŁÓWKI I AKAPITY)
  // ==========================================
  ...
  // ==========================================
  if (type === 'list') {
    newBlock.styles.width = '100%'; newBlock.styles.padding = '20px';
    if (variant === 'classic-ul') { newBlock.text = '<ul style="padding-left: 20px; list-style-type: disc; margin: 0; color: #334155; line-height: 1.8;"><li>Pierwszy, bardzo ważny punkt</li><li>Drugi argument potwierdzający tezę</li><li>Trzeci punkt zamykający temat</li></ul>'; newBlock.styles.fontSize = '16px'; } 
    else if (variant === 'classic-ol') { newBlock.text = '<ol style="padding-left: 20px; list-style-type: decimal; margin: 0; color: #334155; line-height: 1.8;"><li>Załóż darmowe konto</li><li>Skonfiguruj swój profil</li><li>Ciesz się dostępem do platformy</li></ol>'; newBlock.styles.fontSize = '16px'; }
    else if (variant === 'features') { newBlock.text = `<style>.feat-list-${rnd} { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 12px; } .feat-list-${rnd} li { display: flex; align-items: center; gap: 10px; font-size: 16px; color: #1e293b; font-weight: 600; } .feat-list-${rnd} li::before { content: '✓'; display: inline-flex; align-items: center; justify-content: center; width: 24px; height: 24px; background: #dcfce7; color: #10b981; border-radius: 50%; font-size: 14px; flex-shrink: 0; }</style><ul class="feat-list-${rnd}"><li>Nielimitowany dostęp do bazy wiedzy</li><li>Wsparcie techniczne 24/7 Premium</li><li>Darmowe aktualizacje przez rok</li><li>Dedykowany opiekun klienta</li></ul>`; }
    else if (variant === 'steps') { newBlock.styles.backgroundColor = '#f8fafc'; newBlock.styles.borderRadius = '16px'; newBlock.text = `<style>.step-list-${rnd} { list-style: none; padding: 0; margin: 0; display: flex; flex-direction: column; gap: 24px; counter-reset: step-counter_${rnd}; border-left: 3px solid #cbd5e1; margin-left: 20px; padding-left: 30px; } .step-list-${rnd} li { position: relative; } .step-list-${rnd} li::before { counter-increment: step-counter_${rnd}; content: counter(step-counter_${rnd}); position: absolute; left: -46px; top: 0; width: 30px; height: 30px; background: #3b82f6; color: white; border-radius: 50%; display: flex; align-items: center; justify-content: center; font-weight: 900; font-size: 14px; border: 4px solid #f8fafc; } .step-title-${rnd} { font-size: 18px; font-weight: 800; color: #0f172a; margin-bottom: 4px; } .step-desc-${rnd} { font-size: 15px; color: #64748b; line-height: 1.5; }</style><ol class="step-list-${rnd}"><li><div class="step-title-${rnd}">Analiza i strategia</div><div class="step-desc-${rnd}">Dokładnie badamy Twoje potrzeby i ustalamy plan działania.</div></li><li><div class="step-title-${rnd}">Projektowanie UX/UI</div><div class="step-desc-${rnd}">Tworzymy makiety i pełny wizualny prototyp produktu.</div></li><li><div class="step-title-${rnd}">Wdrożenie i testy</div><div class="step-desc-${rnd}">Programujemy, testujemy i odpalamy projekt na produkcji.</div></li></ol>`; }
  }

  if (type === 'faq') { 
    newBlock.styles.width = '100%'; 
    if (variant === 'interactive') { newBlock.text = `<style>.faq-det-${rnd} { background: #ffffff; border: 1px solid #e2e8f0; border-radius: 12px; margin-bottom: 10px; overflow: hidden; transition: all 0.3s ease; } .faq-det-${rnd}[open] { box-shadow: 0 10px 15px -3px rgba(0,0,0,0.05); border-color: #cbd5e1; } .faq-sum-${rnd} { padding: 20px; font-size: 16px; font-weight: 700; color: #0f172a; cursor: pointer; display: flex; justify-content: space-between; align-items: center; list-style: none; } .faq-sum-${rnd}::-webkit-details-marker { display: none; } .faq-sum-${rnd}::after { content: '+'; font-size: 20px; color: #3b82f6; font-weight: 300; transition: transform 0.3s; } .faq-det-${rnd}[open] .faq-sum-${rnd}::after { transform: rotate(45deg); } .faq-content-${rnd} { padding: 0 20px 20px 20px; color: #64748b; font-size: 15px; line-height: 1.6; }</style><div><details class="faq-det-${rnd}"><summary class="faq-sum-${rnd}">Czy mogę zrezygnować w dowolnym momencie?</summary><div class="faq-content-${rnd}">Tak, nasza subskrypcja jest elastyczna. Możesz anulować ją z poziomu panelu użytkownika, a konto wygaśnie wraz z końcem opłaconego miesiąca.</div></details><details class="faq-det-${rnd}"><summary class="faq-sum-${rnd}">Czy oferujecie zniżki dla NGO i uczelni?</summary><div class="faq-content-${rnd}">Oczywiście. Skontaktuj się z naszym działem wsparcia, aby otrzymać specjalny cennik ze zniżką do 50% na wszystkie pakiety.</div></details></div>`; } 
    else { newBlock.text = '▼ Pytanie FAQ<br><br>Odpowiedź.'; newBlock.styles.border = '1px solid #ccc'; newBlock.styles.padding = '15px'; newBlock.styles.backgroundColor = '#fff'; newBlock.styles.width = '100%'; }
  }

  // ==========================================
  // 6. KONTENERY, MENU, ALERTY, BENTO
  // ==========================================
  if (type === 'container') {
  ...
  if (type === 'shape') { 
    if(variant==='box'){newBlock.styles.width='100px'; newBlock.styles.height='100px'; newBlock.styles.backgroundColor='#3b82f6';} 
    if(variant==='circle'){newBlock.styles.width='100px'; newBlock.styles.height='100px'; newBlock.styles.backgroundColor='#ec4899'; newBlock.styles.borderRadius='50%';} 
  }
  ...
  if (type === 'embed') { 
    ...
    newBlock.styles.width = '100%'; newBlock.styles.height = '400px'; newBlock.styles.borderRadius = '12px'; 
  }
  
  // Zgodność wsteczna dla starego gridu
  if (type === 'grid' && variant === 'gallery-grid') { 
    newBlock.styles.gridTemplateColumns = 'repeat(3, 1fr)'; newBlock.styles.gap = '20px'; 
    newBlock.children = [createBlock('img', 'rounded', 'Foto 1'), createBlock('img', 'rounded', 'Foto 2'), createBlock('img', 'rounded', 'Foto 3')]; 
  }
