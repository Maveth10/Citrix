...
export default function ReadyFormsPanel({ handleAddBlock }: { handleAddBlock: any }) {
  return (
    <div className="flex flex-col gap-2 p-2 bg-black/20 rounded-b-xl border border-t-0 border-white/5">
       <UICard 
         icon="📨" 
         title="Formularz Kontaktowy" 
         desc="Gotowy blok: Imię, Email, Wiadomość i Przycisk Wyślij." 
         onClick={() => handleAddBlock('form', 'contact', 'Kontakt')} 
       />
       <UICard 
         icon="🚀" 
         title="Zapis na Newsletter" 
         desc="Kompaktowy układ poziomy. Zbieraj leady błyskawicznie." 
         onClick={() => handleAddBlock('form', 'newsletter', 'Newsletter')} 
       />
       <UICard 
         icon="🔐" 
         title="Karta Logowania" 
         desc="Kompletna sekcja uwierzytelniania (Login & Hasło)." 
