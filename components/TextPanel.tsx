...
export default function TextPanel({ handleAddBlock }: TextPanelProps) {
  ...
  const getTabBtnStyle = (tabName: string) => {
    ...
    return `flex items-center justify-between p-4 transition-all duration-500 font-medium text-[10px] uppercase tracking-[0.2em] border-b relative overflow-hidden z-10 ${
      isActive 
        ? 'bg-gradient-to-r from-[color:var(--theme-color)]/10 to-transparent text-white border-[color:var(--theme-color)]/30' 
        : 'bg-transparent text-neutral-500 hover:text-neutral-300 border-white/5 hover:bg-white/[0.02]'
    }`;
  };
  ...
  return (
    <div className="flex flex-col pb-10 relative z-10">
      ...
      <div className="flex flex-col">
        ...
        <div className={`transition-all duration-500 overflow-hidden ${activeTab === 'headings' ? 'max-h-[1000px] opacity-100' : 'max-h-0 opacity-0'}`}>
      ...
      <div className="flex flex-col">
        ...
        <div className={`transition-all duration-500 overflow-hidden ${activeTab === 'paragraphs' ? 'max-h-[1000px] opacity-100' : 'max-h-0 opacity-0'}`}>
      ...
      <div className="flex flex-col">
        ...
        <div className={`transition-all duration-500 overflow-hidden ${activeTab === 'inserts' ? 'max-h-[1000px] opacity-100' : 'max-h-0 opacity-0'}`}>
