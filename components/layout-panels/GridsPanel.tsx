...
export default function GridsPanel({ handleAddBlock }: PanelProps) {
  return (
    <div className="p-2 bg-black/20 rounded-b-xl border border-t-0 border-white/5">
      <UICard 
        icon="◫" 
        title="Siatka 2-Kolumnowa" 
        desc="Równy podział 50/50. Idealny na tekst po lewej i obraz po prawej." 
        onClick={() => handleAddBlock('grid', 'grid-2', 'Grid 2 Kolumny')} 
      />
      <UICard 
        icon="▦" 
        title="Siatka 3-Kolumnowa" 
        desc="Trzy równe kolumny. Sprawdza się przy cechach lub pakietach cennika." 
        onClick={() => handleAddBlock('grid', 'grid-3', 'Grid 3 Kolumny')} 
      />
      <UICard 
        icon="🍱" 
        title="Bento Grid" 
        desc="Modny, asymetryczny układ: jeden duży kafelek i dwa mniejsze." 
        onClick={() => handleAddBlock('grid', 'bento', 'Bento Grid')} 
      />
    </div>
  );
}