...
export default function CanvasBlock({ 
  ...
  if (!b || hiddenBlocks.includes(b.id)) return null;
  ...
  // Widoczność ukrytych bloków w edytorze
  ...
  return (
    <>
      ...
      <div 
        ...
        className={`group relative ${!isActive && !isPreviewMode ? 'hover:outline hover:outline-1 hover:outline-[#ff4500]/50 hover:outline-dashed' : ''} ${draggedId === b.id ? 'opacity-50' : ''}`}
      >
        ...
        {/* 🚀 ROUTER DO ELEMENTÓW (TEKST, FOTO, PRZYCISKI) */}
        <CanvasElement 
          b={b} 
        ...
        {isActive && !isEditing && !isPreviewMode && ( 
          <ActiveBlockOverlay block={b} currentZIndex={currentZIndex} updateActiveBlock={updateActiveBlock} copiedStyles={copiedStyles} setCopiedStyles={setCopiedStyles} handleDuplicate={handleDuplicate} removeActiveBlock={removeActiveBlock} handleResizeStart={handleResizeStart} /> 
        )}
      </div>
    </>
  );
