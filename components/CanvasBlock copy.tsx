...
export default function CanvasBlock({ 
  ...
  // Zabezpieczenie przed utratą klikalności: Element staje się "ciągnięty" dopiero po ruchu myszki > 3px
  ...
  const containerStyles: any = { 
    ...
    // Gdy w pełni przenosimy obiekt, przepuszczamy kliknięcia na drop-zony pod spodem
    pointerEvents: isBeingDragged ? 'none' : 'auto',
    zIndex: isBeingDragged ? 99999 : (isActive ? 9999 : (currentStyles.zIndex || 1)),
  ...
  // ROZWIĄZANIE WYLEWANIA SIĘ: Usunąłem błąd "delete containerStyles", który niszczył całą siatkę w starym kodzie
  if (b.children) { 
  ...
  const renderTextElement = (Tag: keyof JSX.IntrinsicElements) => {
    ...
    return (
      <Tag style={textStyles} contentEditable={isActive && isEditing} suppressContentEditableWarning={true} onDoubleClick={(e: any) => { e.stopPropagation(); setIsEditing(true); }} onBlur={(e: any) => { setIsEditing(false); updateActiveBlock({ text: e.currentTarget.innerHTML }); }} dangerouslySetInnerHTML={{ __html: b.text || '' }} />
    );
  };
  ...
  return (
    <>
      ...
      {/* SWOBODNE PRZESUWANIE: Czyste JS-owe zdarzenia. D&D HTML5 wyrzucone do kosza. */}
      ...
      >
        ...
        {isActive && !isEditing && (
          <div className="absolute inset-0 pointer-events-none border-2 border-[#ff4500] z-[200]">
            <div className="absolute -top-6 left-[-2px] bg-[#ff4500] text-white text-[9px] px-3 py-1.5 rounded-t font-bold shadow-sm whitespace-nowrap z-[200] flex items-center gap-2 pointer-events-auto cursor-default transition-colors hover:bg-orange-600"
                 onDoubleClick={(e) => { e.stopPropagation(); updateActiveBlock({ styles: { left: '0px', top: '0px' } }, true); }}
                 title="Dwuklik: Wyzeruj swobodne przesunięcie na 0,0"
            >
              <span>{b.name}</span>
            </div>
