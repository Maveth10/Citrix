...
export default function MapsPanel({ handleAddBlock }: { handleAddBlock: any }) {
  return (
    <div className="flex flex-col gap-2 p-2 bg-black/20 rounded-b-xl border border-t-0 border-white/5">
       <UICard 
         icon="🗺️" 
         title="Mapa Google" 
         desc="Wklej interaktywną mapę lokalizacji swojego biznesu." 
         onClick={() => handleAddBlock('embed', 'maps', 'Mapa')} 
       />
    </div>
  );
}