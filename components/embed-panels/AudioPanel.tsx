...
export default function AudioPanel({ handleAddBlock }: PanelProps) {
  return (
    <div className="p-2 bg-black/20 rounded-b-xl border border-t-0 border-white/5">
      <UICard 
        icon="🎧" 
        title="Odtwarzacz Spotify" 
        desc="Osadź podcast, utwór lub całą playlistę ze Spotify." 
        onClick={() => handleAddBlock('embed', 'spotify', 'Spotify')} 
      />
      <UICard 
        icon="☁️" 
        title="SoundCloud Player" 
        desc="Widget dźwiękowy z SoundCloud." 
        onClick={() => handleAddBlock('embed', 'soundcloud', 'SoundCloud')} 
      />
    </div>
  );
}