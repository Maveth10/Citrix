...
export default function StreamingPanel({ handleAddBlock }: { handleAddBlock: any }) {
  return (
    <div className="flex flex-col gap-2 p-2 bg-black/20 rounded-b-xl border border-t-0 border-white/5">
       <UICard 
         icon="📺" 
         title="YouTube Player" 
         desc="Wstaw link do filmu. Obsługuje parametry startowe i auto-play." 
         onClick={() => handleAddBlock('video', 'youtube', 'Wideo YouTube')} 
       />
       <UICard 
         icon="🔹" 
         title="Vimeo Player" 
         desc="Elegancki odtwarzacz bez reklam. Idealny do portfolio." 
         onClick={() => handleAddBlock('video', 'vimeo', 'Wideo Vimeo')} 
       />
    </div>
  );
}