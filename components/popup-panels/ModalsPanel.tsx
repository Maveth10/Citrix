...
export default function ModalsPanel({ handleAddBlock }: { handleAddBlock: any }) {
  return (
    <div className="flex flex-col gap-2 p-2 bg-black/20 rounded-b-xl border border-t-0 border-white/5">
       <UICard 
         icon="🎯" 
         title="Promo Modal" 
         desc="Wielkie, centralne okno (Overlay) ze specjalną ofertą i ciemnym tłem." 
         onClick={() => handleAddBlock('popup', 'modal-promo', 'Promo Modal')} 
       />
       <UICard 
         icon="✉️" 
         title="Newsletter Popup" 
         desc="Okno zachęcające do zostawienia maila (Lead Capture)." 
